...
The immutable base catalogue is stored in the repository as JSON shards under `data/` and is bundled with every deployment. New and revised events are never written back into those source files. Legacy append-only shards under `catalog/updates/` remain readable, but new refreshes are stored in Backblaze B2 as immutable atomic objects under `catalog/daily-v2/`; each object contains both the changed events and the metadata proving that day's check completed. Existing bucket objects do not need to be deleted during migration. Daily forecast candidates are cached under `forecasts/daily-v3.9/` with unique names, and readers choose the candidate with the greatest catalogue coverage so concurrent Vercel instances cannot overwrite a more complete result with a stale one. Older forecast prefixes are left untouched and are not read by the new store.

Once per Türkiye calendar day (UTC+3, at the first request after midnight), the server requests the Sismik Harita earthquake-list API starting 48 hours before the newest stored event. The overlap captures both late arrivals and revisions to recent earthquakes. Stale ranges are split into windows of at most 28 days; incoming records are normalised, deduplicated by stable event identity, and compared with the stored event signature. A successful production refresh requires its atomic daily object to reach B2 before the day is considered checked; failed provider or persistence attempts remain retryable. The application merges the bundled catalogue, legacy shards, and atomic daily objects, keeps the latest revision of each event, and sorts the result newest-first. The CDN freshness lifetime ends at the next Türkiye midnight.
